...
Ubuntu package comes with a proper config file by default (reference appendix src/haproxy_basic.cfg).   
...
\end{lstlisting} 
This configuration defines the application ossec_app on port 80.
...
to configure the master settings (host, log_pos, log_file, user,
...
\section{Setting up the network RAID-1}
...
\cite{drbd_ubuntu_doc} \cite{drbd_official_doc}:
\begin{lstlisting}[language=bash]
resource r0 {
	net {
...
\end{lstlisting}
The node1 and node2 configuration elements are straightforward, I commented the
